...
#PBS -t 1-1%1               # m-n%c   submit the job array indexed from (m) to (n) (inclusive) but only allow c jobs to be running (c)oncurrently.
...
echo Moving to project directory: current_sims
cd current_sims
...
R CMD BATCH code/call_unweighted.R rout/${PBS_JOBID}.Rout --no-save  # run algorithm.r (which can grab the array ID) and save R console output to file numbered by array Id i the rout directory
