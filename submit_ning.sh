...
#PBS -t 1-3%5               # m-n%c   submit the job array indexed from (m) to (n) (inclusive) but only allow c jobs to be running (c)oncurrently.
...
R CMD BATCH code/call_ning.R rout/${PBS_JOBID}.Rout --no-save  # run algorithm.r (which can grab the array ID) and save R console output to file numbered by array Id i the rout directory
